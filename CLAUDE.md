...
**Admin comments:** `spec/comments.yaml` (`TableComments` HTML). Follow **Generate table comments** in conventions (`ask` default). Dated changelog includes the requestor full name. Line comments: `comments.lines` / `comments.subgrids.*.lines`. See [docs/entities/comments.md](docs/entities/comments.md).
