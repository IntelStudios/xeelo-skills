...
2. Prefer project loop: download DB transfer → extract `env/` → edit `changes/<slug>/` → generate Object Transfer. After generate, **dry-run** `--only-test`, then `/publish` per `projects/<name>/conventions.md` (**Publish after dry-run:** `ask` → offer this loop / this+remember / skip; `auto` → run and announce) when site `permission` is `read-write` or `full`. Before `/publish`, always ask whether to backup (`Mutate_admin_transfer_backup`). `/publish` precompiles only at **`full`**. Same for `/download-db` after successful publish. After spec edits, **Generate table comments** the same way (`spec/comments.yaml`).
...
4. Connection: `projects/<project>/.xeelo-connection.json` (gitignored) with `xeeloUrl`, GraphQL `token`, and `permission` (`read-only` | `read-write` | `full`; missing = `read-only`). Optional `userLogin` / `userPwd` for `/ui-test`. Do not change `permission` unless the user asks. See [AGENT.md § Site permission](AGENT.md#site-permission).
...
**Skills** live in [`.agents/skills/`](.agents/skills/). User UI testing lives in [`ui_testing/`](ui_testing/) (`/ui-test` pointer: [`.agents/skills/ui-test/SKILL.md`](.agents/skills/ui-test/SKILL.md)). When the user invokes `/new-project`, asks for a new Xeelo project/site, invokes `/download-db`, or asks to download/refresh/pull a DB transfer or sync env from the site, invokes `/publish` or asks to publish/deploy an Object Transfer, invokes `/precompile` or asks to precompile settings only, invokes `/graphql` or asks to query/mutate GraphQL (`Select_` / `Mutate_`, ticket counts, `access_rights`, header filters such as `createdDate`), invokes `/sync-main` or asks to regularly check/pull/sync **xeelo-skills `main`**, invokes `/ui-test` or asks to UI-test User UI / log in as a Xeelo user (always write a result MP4), **read** the matching `SKILL.md` and follow it **before** product source:
...
- [`ui_testing/SKILL.md`](ui_testing/SKILL.md)

**New project:** after the `projects/` check, create `projects/<name>/` + empty `snapshots/`, `env/`, `changes/` + `.xeelo-connection.json` with **empty** `token`, `"permission": "read-only"` (ask once whether to set `read-write` or `full`), and empty optional `userLogin` / `userPwd`; never copy from other projects. Infer `xeeloUrl` as `https://<name>.xeelo.online/` only when reasonable; then tell the user what to fill in — see [AGENT.md § Creating a new project](AGENT.md). Commit in the nested `projects/` repo, not in xeelo-skills.
...
**New object:** set **`object.directCreate: true`** (`Object.ObjectIsDirectCreate`) unless the user asks otherwise. Inbox Add then skips the object-picker. Do not flip the flag on an existing object unless asked. See [AGENT.md](AGENT.md) and [create-object.md](recipes/create-object.md).

**New object workflow:** always ask whether to create a **new** workflow or **reuse** an existing one (list from site `env/`: object — name — id). Reuse = `workflow.reuse: true`. OT JSON is a delta vs download: unchanged entity rows are omitted even when FKs still reference them. Do not silent-default `workflow.mode: minimal`. **New workflow roles and statuses:** always ask existing vs new for **both** (`env/shared/roles.yaml` and `statuses.yaml`: name — id, Recommended = existing; choices independent); reuse skips both. Existing catalog rows: copy `env/shared` verbatim — do not change name/flags/order. See [AGENT.md § Ask which workflow](AGENT.md#ask-which-workflow).

**New workflow action:** set **`reopenOnSave: open-only-assigned`** (Admin Reopen on Action = Open only (assigned)) unless the user asks otherwise. Do not flip an existing action unless asked. See [add-workflow.md](recipes/add-workflow.md).
