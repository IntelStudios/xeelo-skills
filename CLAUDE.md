...
2. Prefer project loop: download DB transfer → extract `env/` → edit `changes/<slug>/` → generate Object Transfer. After generate, **dry-run** `--only-test`, then `/publish` per `projects/<name>/conventions.md` (**Publish after dry-run:** `ask` → offer this loop / this+remember / skip; `auto` → run and announce) when site `permission` is `read-write` or `full`. `/publish` precompiles only at **`full`**. Same for `/download-db` after successful publish. After spec edits, **Generate table comments** the same way (`spec/comments.yaml`).
