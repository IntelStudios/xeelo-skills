...
| `alwaysDisabled: true` | `ObjectDefaultLineIsDisabled = 1` (Admin Always disabled). Distinct from `extended.disabled`. **Do not** set this on an **adhoc** target (`typeID > 30`) — Refresh is hidden when the control is disabled; adhoc is a suggested value, so treat the line as an input. Omitting the key does not clear an existing `1` — upsert `false`. [object-line-types.md](../entities/object-line-types.md#adhoc-typeid--30) |
