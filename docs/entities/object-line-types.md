...
Helpers `blue()` / `yellow()` / `green()` / `red()` / `dark()` write Metronic-style tokens such as `[Info_Requestor] [Warning_Draft]` / `[Success_Completed]` / `[Primary_Planned]`. Add extra `when RoleID` / `when RequestStatusID` branches when the object gains approval roles or pending statuses. Hide the line on the **form** with layout `alwaysHidden` plus template `alwaysDisabled`; place it on the inbox/subgrid with `valueWidth: 100`. Do **not** use template `hidden: true` for that — that is extended validation, and `alwaysHidden` does not hide onGrid. Spec/generator do **not** emit server calc — after generate, set those two `ObjectDefaultLine` columns **and** an `ObjectDefaultLineCalculationOrder` row on the OT JSON ([server calculations](#server-calculations)). Open requests: GraphQL `withRefresh: true`. Completed rows: write `lines` (`withRefresh: false`) or an update action. Subgrid helpers: same pattern on `ObjectSubDefaultLine` + `ObjectSubDefaultLineCalculationOrder`; `alwaysHidden` is `ObjectSubLineIsHidden` (not yet in extract/generate).
