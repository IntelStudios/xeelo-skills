...
Helpers `blue()` / `yellow()` / `green()` / `red()` / `dark()` write Metronic-style tokens such as `[Info_Requestor] [Warning_Draft]` / `[Success_Completed]` / `[Primary_Planned]`. Add extra `when RoleID` / `when RequestStatusID` branches when the object gains approval roles or pending statuses. Hide the line on the form (`hidden` + `alwaysDisabled`); place it on the inbox with `valueWidth: 100`. Spec/generator do **not** emit server calc — after generate, set those two `ObjectDefaultLine` columns **and** an `ObjectDefaultLineCalculationOrder` row on the OT JSON ([server calculations](#server-calculations)). Open requests: GraphQL `withRefresh: true` (`Save` / `SaveNew` re-runs type **53**). **`WorkflowAction` does not run template Server-String 53** — write the chip in Run Node.js **(Last)** on the **target** step (`Context.Role` / `Context.RequestStatus`) or add a [WorkflowStepCalculation](workflow.md#workflowstepcalculation) type 53. Completed rows: write `lines` (`withRefresh: false`) or an update action.
