...
In spec, write `id{FIELD_CODE}`; generator replaces with `id{ObjectLineID}`. Source binds: `{sourceKey.valueKey}` → `ObjectLineSourceValueBind` (numeric unquoted; else `'bind'`).
