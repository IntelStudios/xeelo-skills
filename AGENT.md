...
| Object actions / Node.js | [object-actions.md](docs/entities/object-actions.md), [nodejs.md](docs/entities/nodejs.md), [recipes/add-object-action.md](recipes/add-object-action.md), [nodejs-graphql-patterns.md](recipes/nodejs-graphql-patterns.md) |
...
Object Transfer **upserts** by Orig. ID — it does not delete for most tables. Soft-disable with `isActive: false` on the same ID (`ObjectUpdateAction`, `ObjectAction`, …). Omit the row and the site copy stays active. **Exception:** `ObjectLineOnGrid` / `ObjectSubLineOnGrid` — unused placements must be in the JSON with **`IsActive: 0`** so upload **deletes** them (insert keeps `IsActive = 1` only). Omitting a dropped column leaves the live row and it can overlap the new layout. Generator emits those inactive rows from leftover `ids.explicit.objectLineOnGrid` / `subgridOnGrid`. Do not delete those explicit keys in the same change. Cells on one letter must not overlap ([ongrid.md](docs/entities/ongrid.md#no-overlap)).
...
On one `row` letter, columns must **not overlap** (half-open: next `position` ≥ previous `position + length`; lengths still sum to 100). One field or system line per layout variant. Dropped placements stay on site if omitted from OT — generate sends leftover `ids.explicit.objectLineOnGrid` / `subgridOnGrid` Orig. IDs with **`IsActive: 0`** so upload deletes them. Do not remove those explicit keys until after `/download-db`. Same rules for `subgrids.<key>.onGrid`. Details: [ongrid.md](docs/entities/ongrid.md#no-overlap).
...
- [ ] Notifications: `spec/notifications.yaml` bound from workflow / step action / ObjectAction `spNotificationDataInsert` / Periodic `…Insert` or `…Summary`; `{idXXXX}` is numeric `ObjectLineID`; summary body `{RequestGrid,…}` ([add-notification.md](recipes/add-notification.md))
- [ ] **New line (including type-5 subgrid):** emit `templates[].access` visible+editable on create **and** `workflow.steps[].access` on every step that should show it. OT does not run access refresh; a missing widget row hides the field; missing **column** (`ObjectSubLine`) rows leave the add-row modal empty — generator copies parent access onto columns when `objectSub:` is in spec ([add-subgrid.md](recipes/add-subgrid.md#access), [object-model.md](docs/entities/object-model.md#create-form-access-objectdefaultaccess))
- [ ] **New type-5 subgrid:** also emit `subgrids.<key>.onGrid` (`fields` + `layouts`) like request inbox onGrid (no overlap; unused `subgridOnGrid` IDs → `IsActive: 0`); after generate patch `ObjectDefaultLineCalculationOrder` for the type-5 parent (and any Server-SubConcat / other server calc) — [add-subgrid.md](recipes/add-subgrid.md#ongrid), [object-line-types.md](docs/entities/object-line-types.md#calculation-order)
- [ ] **Subgrid columns:** same extras as ObjectLine (`precision` on number, `reference` on combo/radio/multi, `attachmentStorageId`, `previewField`, lookup + `sourceField` in the same tree, client-calc on `subgrids.*.templates[].fields`). Modal **width 80** unless asked; more fields → tabs/sections. Czech in `languageTable.subgrids.<key>`. No `subgrid` / `report` / `button` under `subgrids:` — [object-line-types.md](docs/entities/object-line-types.md#subgrid-columns-objectsubline)
- [ ] Existing lines: `workflow.steps[].access` when a line must be editable after create ([add-workflow.md](recipes/add-workflow.md)); `templates[].access` only to hide or lock on create (not `hidden` / `alwaysDisabled`)
