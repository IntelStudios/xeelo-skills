...
- [ ] **New type-5 subgrid:** also emit `subgrids.<key>.onGrid` (`fields` + `layouts`) like request inbox onGrid; after generate patch `ObjectDefaultLineCalculationOrder` for the type-5 parent (and any Server-SubConcat / other server calc) — [add-subgrid.md](recipes/add-subgrid.md#ongrid), [object-line-types.md](docs/entities/object-line-types.md#calculation-order)
