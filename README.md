...
3. Per project (`projects/<name>/` = one Xeelo):
   - Write `projects/<name>/.xeelo-connection.json` with `xeeloUrl`, GraphQL `token`, and `permission` (`read-only` | `read-write` | `full`; missing = `read-only`; optional `userLogin` / `userPwd` for `/ui-test`)
